...
- **ModelScope API token**：用于向量嵌入（vector projection）。未配置时 vector 维度会失败，但不阻塞章节 commit。
...
3. 初始化项目：
   ...
   ```
4. 将本 skill 的 `SKILL.md` 放入 `.claude/skills/webnovel-full-write/` 目录
...
```
Phase 0: 预检 ─── placeholder 扫描、合同就绪检查
Phase 1: 加载 ─── 17+ 设定文件 + 生成五段式写作任务书
Phase 2: 起草 ─── 续写正文，保存到 正文/第000N章.md
Phase 3: 审查 ─── 8 维度审查 + 自动修复
